...
all_time = 0
MYEVENTTYPE = 30
flag = input('Здравствуйте! это вход в игру PacMan.\nВы хотите зарегистрировать '
             'новый аккаунт (ответом служит строка\nвида "да" или "нет"'
             ' в любом регистре)? ')
while flag.lower() != 'да' and flag.lower() != 'нет':
    flag = input('Я жду от вас строку вида "да" или "нет" в любом регистре. ')
file = open(r"data/passwords.txt")
logins, passwords = [], []
for line in file:
    q = line.strip().split()
    logins += [q[0]]
    passwords += [q[1]]
if flag.lower() == 'да':
    nick_name = input('Введите новое игровое имя (оно не должно содержать знак пробела): ')
    while ' ' in nick_name or nick_name in logins:
        nick_name = input('Повторите попытку ввода игрового имени\n(либо такой пользователь'
                          ' уже зарегистрирован, либо вы использовали пробел): ')
    password = input('Введите пароль (он не должен содержать знак пробела): ')
    while ' ' in password:
        password = input('Повторите попытку ввода пароля, но уже без пробела: ')
    print('Регистрация нового пользователя выполнена успешно, удачной игры!')
    logins += [nick_name]
    passwords += [password]
    s = open(r"data/passwords.txt", 'w')
    for i in range(len(logins)):
        s.write(logins[i] + ' ' + passwords[i] + '\n')
    s.close()
else:
    nick_name = input('Введите своё игровое имя: ')
    while not nick_name in logins:
        nick_name = input('Неправильное игровое имя. Повторите попытку: ')
    password = input('Введите пароль: ')
    while not password in passwords:
        password = input('Неправильный пароль. Повторите попытку: ')
    print('Вход выполнен успешно, удачной игры!')
...
# 1 x ghost pacman
size1 = width_all, height_all = 448, 288 + 32
size = 32

# 1 x 1 pacman
size2 = width_all2, height_all2 = 560, 620 + 64
size22 = 20

# score table
size_score = width_score, height_score = 560 * 2, 310 + 32


def print_points(x, h, s, pacman_num, game='1'):
    font = pygame.font.Font(None, 45)
    text = font.render(str(x) + ' points', 1, (255, 255, 100))
    if game == '1':
        screen.blit(text, (100, h - s))
    elif pacman_num == '1':
        screen.blit(text, (300, h - s))
    else:
        screen.blit(text, (100, h - s))


def print_score(mesto, time, nick):
    font = pygame.font.Font(None, 45)
    text = font.render(str(mesto) + '. Игрок ' + nick + ' прошёл игру за ' + str(time)
                       + ' секунд', 1, (255, 255, 100))
    screen1.blit(text, (50, (mesto - 1) * 60 + 30))
...
class EndWin(pygame.sprite.Sprite):
    ...
    def __init__(self, group):
        global nick_name, all_time
        super().__init__(group)
        self.image = EndWin.End_image
        self.rect = self.image.get_rect()
        self.rect.x = -width_all
        self.rect.y = 0
        file = open(r"data/score_table.txt")
        scores = []
        h = True
        for line in file:
            q = line.strip().split()
            if nick_name == ' '.join(q[1:]) and all_time < q[0]:
                scores += [(all_time / 100, nick_name)]
            elif nick_name == ' '.join(q[1:]):
                h = False
                scores += [(float(q[0]), ' '.join(q[1:]))]
            else:
                scores += [(float(q[0]), ' '.join(q[1:]))]
        if h:
            scores += [(all_time / 100, nick_name)]
        scores.sort()
        s = open(r"data/score_table.txt", 'w')
        for i in range(5):
            s.write(str(scores[i][0]) + ' ' + scores[i][1] + '\n')
        s.close()
...
class ButtonGame1(pygame.sprite.Sprite):
    ...
    Button = pygame.transform.scale(Button, (360, 160))
    ...
    def update(self, *args):
        ...
        if args and args[0].type == pygame.MOUSEBUTTONDOWN and self.rect.collidepoint(args[0].pos):
            ...
            Button_score_table.rect.x = -Button_score_table.rect.w
            Button_score_table.rect.y = -Button_score_table.rect.h
            game_1()
...
class ButtonScoreTable(pygame.sprite.Sprite):
    Button = load_image("button2.jpg", -1)
    Button = pygame.transform.scale(Button, (360, 160))

    def __init__(self, group):
        super().__init__(group)
        self.image = ButtonScoreTable.Button
        self.rect = self.image.get_rect()

    def update(self, *args):
        global running1, Button_score_table
        if args and args[0].type == pygame.MOUSEBUTTONDOWN and self.rect.collidepoint(args[0].pos):
            running1 = False
            Button_game_1.rect.x = -Button_game_1.rect.w
            Button_game_1.rect.y = -Button_game_1.rect.h
...
class ButtonQuit(pygame.sprite.Sprite):
    ...
    def __init__(self, group, game='1'):
        ...
        self.rect = self.image.get_rect()
        if self.game == '1':
            self.rect.y = height_all - 30
        elif self.game == '2':
            self.rect.y = height_all2 - 30
        elif self.game == 'menu':
            self.rect.y = height_start - 30
        else:
            self.rect.y = height_score - 30
...
def game_1():
    global level, END, width_all, height_all, width, height, running, size, all_sprites, \
        Ghosts_sprites, Eat_sprites, Pacman_sprites, screen, pacman, ghost, fps, clock, \
        end_lose, end_win, running1, all_time, MYEVENTTYPE
    all_time = 0
    ...
    while running:
        ...
        screen.fill((0, 0, 0))
        all_sprites.update()
        print_points(pacman.points, height_all, size, '1')
...
def game_2():
    global level, END, width_all2, height_all2, width, height, running, size, all_sprites, \
        ...
        fps, clock, end_lose, end_win, flag_game_2, level_to_play, running1
    all_sprites = pygame.sprite.Group()
    running1 = False
    ...
    for i in range(height):
        for j in range(width):
            if level[i][j] == "^":
                ...
                pacman = PacMan(all_sprites, load_image("pac1 for 1x1.png", -1), 5, 1, j * size22, i * size22, '2')
                Pacman_sprites.add_internal(pacman)
    ...
    quit_button = ButtonQuit(all_sprites, '2')
    ...
    while running:
        ...
        screen.fill((0, 0, 0))
        all_sprites.update()
        print_points(pacman.points, height_all2, size22 + 10, '1', '2')
        print_points(pacmanWASD.points, height_all2, size22 + 10, '2', '2')
        all_sprites.draw(screen)
        pygame.display.flip()
        if not END:
            if check_win():
...
def score_table():
    global running2, scores, width_score, height_score, running, size_score, all_sprites, running1, \
        Button_sprites2, screen1
    all_sprites = pygame.sprite.Group()
    running1 = False
    running2 = True
    screen1 = pygame.display.set_mode(size_score)
    file = open(r"data/score_table.txt")
    scores = []
    cou = 0
    for line in file:
        cou += 1
        q = line.strip().split()
        print_score(cou, q[0], ' '.join(q[1:]))
    running = True
    Buttons_sprites2 = pygame.sprite.Group()
    quit_button = ButtonQuit(Buttons_sprites2, 'score')
    while running2:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                running1 = False
            Buttons_sprites2.update(event)
        Buttons_sprites2.draw(screen1)
        pygame.display.flip()


def main():
    global size_start, width_start, height_start, screen1, running1, running, \
        Buttons_sprites, Button_game_1, Button_game_2, flag_game_2, Button_score_table, nick_name
    running1 = True
    running = False
    Buttons_sprites = pygame.sprite.Group()
    quit_button = ButtonQuit(Buttons_sprites, 'menu')
    quit_button.rect.y = height_start - 30
    Button_game_1 = ButtonGame1(Buttons_sprites)
    Button_game_1.rect.x = width_start // 2 - Button_game_1.rect.w // 2
    Button_game_1.rect.y = 40

    Button_game_2 = ButtonGame2(Buttons_sprites)
    Button_game_2.rect.x = width_start // 2 - Button_game_2.rect.w // 2
    Button_game_2.rect.y = height_start // 2 - Button_game_2.rect.h // 2

    Button_score_table = ButtonScoreTable(Buttons_sprites)
    Button_score_table.rect.x = width_start // 2 - Button_score_table.rect.w // 2
    Button_score_table.rect.y = height_start - Button_score_table.rect.h - 30
