...
# 1 x 1 pacman
size2 = width_all2, height_all2 = 448, 288
size22 = 16
...
class Player1Win(pygame.sprite.Sprite):
    ...
    Player1win = pygame.transform.scale(Player1win, (width_all, height_all))
...
class Player2Win(pygame.sprite.Sprite):
    ...
    Player2win = pygame.transform.scale(Player2win, (width_all, height_all))
...
def game_2():
    global level, END, width_all, height_all, width, height, running, size, all_sprites, \
        ...
        fps, clock, end_lose, end_win, flag_game_2

    screen = pygame.display.set_mode(size1)

    file = open(r"data/level2")
    level = []
    for line in file:
        level += [list(line.strip())]
    width = len(level[0])
    ...
    for i in range(height):
        for j in range(width):
            if level[i][j] == "^":
                ...
                pacmanWASD = PacManWASD(all_sprites, load_image("pac1.png", -1), 5, 1, j * size, i * size)
                Pacman_sprites.add_internal(pacmanWASD)
                pacmanWASD.rect.x, pacmanWASD.rect.y = j * size, i * size
...
def main():
    ...
    Button_game_1.rect.y = height_start // 3 - Button_game_1.rect.h
